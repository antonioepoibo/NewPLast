...
<!-- Existing Subscribe/Creator Logic -->
      <button
        v-if="!activity.subscribed && activity.owner !== sessionStore.userId"
        @click="subscribeToActivity(activity.id)"
        class="bg-green-600 text-white py-2 text-[14px] px-6 font-bold rounded-full"
      >
        Subscribe
      </button>
